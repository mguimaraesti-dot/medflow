...
- **Convite de novo usuário** (Gestão de Acessos → "Novo Usuário") chama `supabaseAdmin.auth.admin.inviteUserByEmail` (`create-user.use-case.ts`), que dispara o e-mail de convite pelo próprio Supabase Auth.
  - O envio desse e-mail passa por **SMTP customizado configurado no painel do Supabase** (Authentication → Emails → SMTP Settings) — o serviço de e-mail embutido do Supabase é fortemente limitado por hora e não serve para uso real.
  - Provedor atual: **Gmail** (`smtp.gmail.com`), com a conta `medflowmae@gmail.com` como remetente. Uma tentativa anterior com Resend (`smtp.resend.com`) foi descartada — o domínio de teste do Resend (`onboarding@resend.dev`) só entrega para o e-mail da própria conta Resend, então não servia para convidar usuários reais sem antes verificar um domínio próprio.
  - Risco conhecido: Gmail via SMTP direto tem um limite de envio diário baixo (na ordem de centenas de e-mails/dia por conta) e depende de uma senha de app (App Password) gerada na conta pessoal — adequado para o volume atual (MVP mono-clínica), mas não é a solução recomendada se o volume de convites crescer; nesse caso, um provedor transacional dedicado com domínio verificado (Resend, SendGrid, Postmark) seria o próximo passo.
...
| Gmail (SMTP) | Envio dos e-mails de convite/recuperação de senha do Supabase Auth | Supabase Auth chama (configurado no painel do Supabase, fora do repositório) | Nenhuma no projeto Next.js — credenciais SMTP (conta `medflowmae@gmail.com` + senha de app) ficam só no painel do Supabase |
...
flowchart TB
    ...
    Google["Google OAuth"]
    GmailSMTP["Gmail<br/>(SMTP customizado)"]
    ...
    Auth -->|e-mail de convite/recuperacao| GmailSMTP
```
