...
export function StatusReportCofreScreen() {
  ...
  const sendWhatsApp = useSendStatusReportCofreWhatsApp();

  const range = computePeriodRange(periodPreset, periodCustom);
  const imageUrl = `/api/reports/status-report-cofre/image?dateFrom=${range.from.toISOString()}&dateTo=${range.to.toISOString()}`;
